...
if (isset($_GET['master'])) {
	...
	if (!ctype_alnum($_GET['master'])) {
		$res['error'] = 'Invalid Master';
		jsonOut($res);
	}
	else {
		if (!file_exists('masters/' . $_GET['master']) || !in_array($_GET['master'], $masters)) {
	...
	if (!ctype_alnum($_POST['username'])) {
		...
		if (!file_exists('masters/' . $_POST['username']) || !in_array($_POST['username'], $masters)) {
			$res['error'] = 'Master not found';
			jsonOut($res);
		}
	}
...
if (isset($_POST['action']) && !empty($_POST['username'])) {
	...
	if ($_POST['action'] == 'update-master') {
		...
		if (!empty($_POST['package'])) {
			error_log($_POST['package'], 3, 't.log');
			$whmParams = array();
			$whmParams['user'] = $_POST['username'];
			$whmParams['pkg'] = $_POST['package'];
			$result = $acc->whmapi('changepackage', $whmParams);
			$res = $acc->updatemaster($_POST['username'], array('pkgname' => $_POST['package']));
		}
	}
	else {
